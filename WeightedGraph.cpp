//Lab 5 is an implmentation of a weighted graph
#include <iostream>
#include <limits>
using namespace std;
...
void Graph::insert_edge(int x, int y, int weight, bool directed) {
    if (x > 0 && x < (MAXV + 1) && y > 0 && y < (MAXV + 1)) {
        Vertex* edge = new Vertex(y, weight);
...
void shortest_path(Graph* g, int parent[], int distance[], int start) {
...
int main() {
    ...
    shortest_path(g, parent, distance, start);
